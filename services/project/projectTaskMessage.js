const ProjectTaskMessage = require("../../models/project/ProjectTaskMessage");
...
module.exports.findOne = async (messageId) => {
  ...
  const message = await ProjectTaskMessage.findById(messageId);
  if (!message) throw Error("Invalid task message");
  return message;
};

module.exports.findAll = async () => {
  const messages = await ProjectTaskMessage.find({});
  if (!messages) throw Error("Invalid task messages");
  return messages;
};

module.exports.saveOne = async (msg, image, user, task) => {
  const message = await ProjectTaskMessage.create({ msg, image, user, task });
...
module.exports.updateOne = async (messageId, messageObj) => {
  if (!messageId || !mongoose.Types.ObjectId.isValid(messageId)) throw Error("Invalid task message id");
  const message = await ProjectTaskMessage.findByIdAndUpdate(messageId, { ...messageObj }, { new: true });
...
module.exports.deleteOne = async (messageId) => {
  if (!messageId || !mongoose.Types.ObjectId.isValid(messageId)) throw Error("Invalid task message id");
  const message = await ProjectTaskMessage.findByIdAndDelete(messageId);
